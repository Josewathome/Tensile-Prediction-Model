...
class DataCollection:
    def __init__(self, master):
        ...
        master.title("CONCRETE RATIO 1:2:4")
        ...
        ttk.Label(master, text="Mass of Cement (g) eg 134.7:").grid(row=0, column=0, sticky=tk.W)
        self.cement_amount = ttk.Entry(master)
        self.cement_amount.grid(row=0, column=1, sticky=tk.W)

        ttk.Label(master, text="Mass of water(g) eg 134.7:").grid(row=1, column=0, sticky=tk.W)
        self.water_mass = ttk.Entry(master)
        self.water_mass.grid(row=1, column=1, sticky=tk.W)

        ttk.Label(master, text="Type of Cement(dropdown menu):").grid(row=2, column=0, sticky=tk.W)
        self.cement = ttk.Combobox(master, values=self.cement_types, state="readonly")
        self.cement.grid(row=2, column=1, sticky=tk.W)

        ttk.Label(master, text="Average Aggregate Size (mm) eg 5 :").grid(row=3, column=0, sticky=tk.W)
        self.aggregate_size = ttk.Entry(master)
        self.aggregate_size.grid(row=3, column=1, sticky=tk.W)

        ttk.Label(master, text="Mass of Aggregate Coarse (g) eg 134.7 :").grid(row=4, column=0, sticky=tk.W)
        self.Coarse_aggregate = ttk.Entry(master)
        self.Coarse_aggregate.grid(row=4, column=1, sticky=tk.W)

        ttk.Label(master, text="Mass of Sand (g) eg 134.7:").grid(row=5, column=0, sticky=tk.W)
        self.mass_sand = ttk.Entry(master)
        self.mass_sand.grid(row=5, column=1, sticky=tk.W)

        ttk.Label(master, text="Curing Duration (days) eg 7:").grid(row=6, column=0, sticky=tk.W)
        self.curing_duration = ttk.Entry(master)
        self.curing_duration.grid(row=6, column=1, sticky=tk.W)

        ttk.Label(master, text="Admixtures(dropdown menu):").grid(row=7, column=0, sticky=tk.W)
        self.admixture = ttk.Combobox(master, values=self.admixtures, state="readonly")
        self.admixture.grid(row=7, column=1, sticky=tk.W)
        ...
        self.NAMES_1.grid(row=10, column=1, columnspan=2)

        self.NAMES_2 = ttk.Label(self.master, text="EO33-01-1386/2020 JOSEPH. G. WATHOME")
        self.NAMES_2.grid(row=11, column=1, columnspan=2)
    ...
    # Collect the data
        ...
        self.prediction_label.grid(row=9, column=0, columnspan=2)
